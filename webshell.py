...
import socket, os, sys, pty, signal, select, gzip,io
import threading, fcntl, termios, struct, pwd, subprocess
import  mimetypes
...
class Terminal:
	...
	def reset_hard(self):
		...
		#	B:	Background 
		# 
		# oxfe0000 represent Unicode private use area 
		# it contains  65536 codes
	...
	def reset_screen(self):
		# Screen 
		# 0x00fe0000 | 0x20 = fe0020 = Zero Width Space
		...
		for c in d:
			...
			if self.utf8_units_count != self.utf8_units_received:
				...
				if (char & 0xc0) == 0x80:
					...
					if self.utf8_units_count == self.utf8_units_received:
						if self.utf8_char<0x10000: # three bytes utf8 represent Unicode
				...
				# first byte which start was 0 (1 byte char)
				if (char & 0x80) == 0x00:
					o += c
				# first byte which start was 110(2 bytes char)
				elif (char & 0xe0) == 0xc0:
					self.utf8_units_count = 1
					self.utf8_char = char & 0x1f 
				# first byte which start was 1110 (3 bytes char) 
				elif (char & 0xf0) == 0xe0:
					self.utf8_units_count = 2
					self.utf8_char = char & 0x0f 
				# first byte which start was 11110(4 bytes char)
				elif (char & 0xf8) == 0xf0:
			...
			for x in range(0, self.w):
				...
				if attr != attr_:
					...
					if attr & 0x0100:
						...
						ul = ''
					dump += '<span class="f%x b%x%s">' % (fg, bg, ul)
					attr_ = attr
...
class Multiplex:
	# given a cmd and term_type default are localhost xterm
...
#			self.proc_finish(sid)
			...
			events = EVENT_READ
			selector.register(fd, events, data=sid)
			# send terminal size to fd
			try:
				fcntl.ioctl(fd, 
					struct.unpack('i', 
						struct.pack('I', termios.TIOCSWINSZ)
	...
	def proc_dump(self, sid):
		if sid not in self.session:
			return False
		return self.session[sid]['term'].dump().encode('utf8')
	...
	def proc_thread(self):
		while not self.signal_stop: 
			(fds, _) = self.proc_getalive() 
			for fd in fds:  
				if not selector.get_key(fd): 
